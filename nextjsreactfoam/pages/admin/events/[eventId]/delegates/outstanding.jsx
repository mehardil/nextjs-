...
const OutstandingDelegatesReport = ({
  event,
  templates,
  filters,
  delegates,
  ...
  if (session) {
    try {
      ...
      const delegateFiltersResponse = await api({
        url: `/events/${context.params.eventId}/registrations/filters/checked`,
        headers,
      });

      const delegatesResponse = await api({
        url: `/events/${context.params.eventId}/registrations?isChecked=1${
          !isEmpty(urlQueryString) ? `&${urlQueryString}` : ""
        }`,
        headers,
      });
...
OutstandingDelegatesReport.Layout = ViewEventLayout;

export default OutstandingDelegatesReport;
