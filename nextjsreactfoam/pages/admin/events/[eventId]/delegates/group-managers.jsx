  ...
  if (session) {
    try {
      ...
      const delegateFiltersResponse = await api({
        url: `/events/${context.params.eventId}/registrations/filters/group-managers`,
        headers,
      });

      const delegatesResponse = await api({
        url: `/events/${context.params.eventId}/registrations?isGroupManager=1${
...
GroupManagersReport.Layout = ViewEventLayout;

export default GroupManagersReport;
