...
const FinanceProcessedDD = ({ event, transactions }) => {
...
export const getServerSideProps = async (context) => {
  ...
  if (session) {
    ...
    const tableResponse = await api({
      url: `/events/${context.params.eventId}/finance/reports/transactions?type=DirectDebit&Complete=0`,
      method: "GET",
      headers: {
        Authorization: `Bearer ${session.accessToken}`,
        "X-Tenant": getTenant(context.req),
...
FinanceProcessedDD.authorized = true;
FinanceProcessedDD.allowedRoles = [ADMIN];
FinanceProcessedDD.Layout = ViewEventLayout;
FinanceProcessedDD.defaultProps = {
  transactions: [],
};

export default FinanceProcessedDD;
