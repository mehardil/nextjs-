...
const SubmissionUserSubmissions = ({
  event,
  submission,
  filters,
  userSubmissions,
  ...
  if (session) {
    try {
      ...
      const userSubmissionsResponse = await api({
        url: `/events/${context.params.eventId}/submissions/${context.params.submissionId}/user-submissions?withUserData=1&withCategoryData=1&withPresentationTypeData=1&withSessionData=1&${urlQueryString}`,
        headers: {
          Authorization: `Bearer ${session.accessToken}`,
          "X-Tenant": getTenant(context.req),
        },
...
SubmissionUserSubmissions.Layout = ViewEventSubmissionLayout;

export default SubmissionUserSubmissions;
