  ...
  if (session) {
    try {
      ...
      const filtersResponse = await api({
        url: `/events/${context.params.eventId}/user-submissions/filters/removed`,
        headers,
      });

      const userSubmissionsResponse = await api({
        url: `/events/${context.params.eventId}/submissions/${context.params.submissionId}/user-submissions?removed=1&withUserData=1&withCategoryData=1&withPresentationTypeData=1&withSessionData=1&${urlQueryString}`,
        headers: {
          Authorization: `Bearer ${session.accessToken}`,
          "X-Tenant": getTenant(context.req),
        },
...
RemovedUserSubmission.Layout = ViewEventSubmissionLayout;

export default RemovedUserSubmission;
