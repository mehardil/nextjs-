...
import ReviewsTable from "@/components/common/ReviewsTable";
  ...
  if (session) {
    try {
      ...
      const filtersResponse = await api({
        url: `/events/${context.params.eventId}/user-submissions/filters/reviews`,
        headers,
      });

      const userSubmissionsResponse = await api({
        url: `/events/${context.params.eventId}/submissions/${context.params.submissionId}/user-submissions?reviews=1&withUserData=1&withCategoryData=1&withPresentationTypeData=1&withSessionData=1&${urlQueryString}`,
        headers: {
          Authorization: `Bearer ${session.accessToken}`,
          "X-Tenant": getTenant(context.req),
        },
...
ReviewsUserSubmission.Layout = ViewEventSubmissionLayout;

export default ReviewsUserSubmission;
