  ...
  return (
    <ContentErrorBoundary>
      ...
      <FilterLayout filters={filters} templates={[]} item={event?.id}>
        {() => (
          <Panel
            title={t("common.author", { entries: 1 })}
            color="inverse"
            maximizable
            collapsable
          >
  ...
  if (session) {
    try {
      ...
      const filtersResponse = await api({
        url: `/events/${context.params.eventId}/user-submissions/filters/authors`,
        headers,
      });

      const userSubmissionsResponse = await api({
        url: `/events/${context.params.eventId}/submissions/${context.params.submissionId}/user-submissions?authors=1&withUserData=1&withCategoryData=1&withPresentationTypeData=1&withSessionData=1&${urlQueryString}`,
        headers: {
          Authorization: `Bearer ${session.accessToken}`,
          "X-Tenant": getTenant(context.req),
        },
...
AuthorUserSubmission.Layout = ViewEventSubmissionLayout;

export default AuthorUserSubmission;
