...
import SharingPartnerTable from "@/components/common/AccommodationsReports/SharingPartnerTable";

const AccommodationsSharingPartner = ({
  event,
  filters,
  context,
  sharingPartner,
  ...
  return (
    <ContentErrorBoundary>
      ...
      >
        {() => (
          <Panel
            title="Sharing Partner"
            color="inverse"
            maximizable
            collapsable
          >
            <SharingPartnerTable items={sharingPartner} />
          </Panel>
        )}
      </FilterLayout>
    </ContentErrorBoundary>
...
export async function getServerSideProps(context) {
  ...
  if (session) {
    try {
      ...
      const sharingPartnerFiltersResponse = await api({
        url: `/events/${context.params.eventId}/reports/filters/sharing-partner-report`,
        headers,
      });
      return {
        props: {
          ...
          filters: sharingPartnerFiltersResponse.data,
        },
      };
    } catch (e) {
      return {
...
AccommodationsSharingPartner.Layout = ViewEventLayout;

export default AccommodationsSharingPartner;
