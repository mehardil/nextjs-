...
import BlockingTable from "@/components/common/AccommodationsReports/BlockingTable";

const AccommodationsBlocking = ({ event, filters, context, blocking }) => {
  ...
  return (
    <ContentErrorBoundary>
      ...
      >
        {() => (
          <Panel title="Blocking" color="inverse" maximizable collapsable>
            <BlockingTable items={blocking} />
          </Panel>
        )}
      </FilterLayout>
    </ContentErrorBoundary>
...
export async function getServerSideProps(context) {
  ...
  if (session) {
    try {
      ...
      const blockingReportFiltersResponse = await api({
        url: `/events/${context.params.eventId}/reports/filters/blocking-report`,
        headers,
      });
      return {
        props: {
          ...
          filters: blockingReportFiltersResponse.data,
        },
      };
    } catch (e) {
      return {
...
AccommodationsBlocking.Layout = ViewEventLayout;

export default AccommodationsBlocking;
