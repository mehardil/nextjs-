...
const AccommodationsDelegates = ({ event, filters, context, delegates }) => {
      ...
      >
        {() => (
          <Panel title="Delegates" color="inverse" maximizable collapsable>
            <DelegatesTable items={delegates} />
          </Panel>
        )}
      </FilterLayout>
    </ContentErrorBoundary>
...
export async function getServerSideProps(context) {
  ...
  if (session) {
    try {
      ...
      const delegatesReportFiltersResponse = await api({
        url: `/events/${context.params.eventId}/reports/filters/delegates-report`,
        headers,
      });
      return {
        props: {
          ...
          filters: delegatesReportFiltersResponse.data,
        },
      };
    } catch (e) {
      return {
...
AccommodationsDelegates.Layout = ViewEventLayout;

export default AccommodationsDelegates;
