...
import OrganisationSetupMembershipDetail from "@/components/forPage/OrganisationSetupWizard/OrganisationSetupMembershipDetail";
...
import {
  updateOrgaFinancial,
  updateOrgaMembership,
} from "@/requests/organization";

const AddOrgMembershipIndexPage = ({
  authUser,
  Organization,
  tenant,
  session,
  ...
  const handleSubmit = async (params, { redirectToNextStep }) => {
    ...
    try {
      await updateOrgaMembership(Organization?.id, params, session.accessToken);
  ...
  return (
    <>
      ...
      <FormProvider {...methods}>
        ...
        >
          <OrganisationSetupMembershipDetail isSaving={isLoading} />
        </Wizard>
      </FormProvider>
    </>
  );
...
AddOrgMembershipIndexPage.authorized = true;
AddOrgMembershipIndexPage.Layout = DashboardLayout;

export default AddOrgMembershipIndexPage;
