...
import OrganisationSetupAddons from "@/components/forPage/OrganisationSetupWizard/OrganisationSetupAddons";
...
import { updateOrgaAddon } from "@/requests/organization";

const AddOrgAddonIndexPage = ({ authUser, Organization, tenant, session }) => {
  ...
  const handleSubmit = async (params, { redirectToNextStep }) => {
    ...
    try {
      await updateOrgaAddon(Organization?.id, params, session.accessToken);
  ...
  return (
    <>
      ...
      <FormProvider {...methods}>
        ...
        >
          <OrganisationSetupAddons isSaving={isLoading} />
        </Wizard>
      </FormProvider>
    </>
  );
...
AddOrgAddonIndexPage.authorized = true;
AddOrgAddonIndexPage.Layout = DashboardLayout;

export default AddOrgAddonIndexPage;
