...
import MembershipAddonItemTable from "@/components/common/MembershipAddonItemTable";
...
const addonItems = ({ membership, context }) => {
  ...
  const pageHeaderToolbar = (
    <Button
      ...
      isCircle
      onClick={() => {
        setAddonInvoiceModal(true);
      }}
    >
      <Icon icon="plus" className="mr-1" />
      {t("common.forms.addNewEntity", {
        entity: t("common.addonInvoice", { entries: 2 }),
      })}
    </Button>
  );
  return (
    <ContentErrorBoundary>
      <PageHeader title="Addon Items" toolbar={pageHeaderToolbar} />
      {addoninvoiceModal && (
        <AddonInvoiceModal
          isShow={addoninvoiceModal}
          onHide={() => setAddonInvoiceModal(false)}
        />
      )}

      <Panel title="Addon Items" color="inverse">
        <MembershipAddonItemTable />
      </Panel>
    </ContentErrorBoundary>
  );
};
